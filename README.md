...
- **Custom prior**: informative prior over skill profiles via
  `cdcat_prior()`
- **Fixed or adaptive start item**: control the first item via
  `start_item`
...
- **Built-in simulated datasets**: `cdcat_sim` — 16 datasets across K,
...
(HD-LV, HD-HV, LD-LV, LD-HV). Each dataset contains the Q-matrix, true
attribute profiles, item parameters, and simulated responses.
